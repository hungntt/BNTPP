...
def process_seq(dataset_dir, sub_datasets, num_samples, process_dim, fold=None):
    ...
    for i, sub_dataset in enumerate(sub_datasets):
        save_path = f'{dataset_dir}{sub_dataset}_manifold_format{fold}.pkl'
        ...
        with open(save_path, "wb") as f:
            save_data_ = {'timestamps': seq_times[np.sum(num_samples_[:i + 1]):np.sum(num_samples_[:i + 2])],
                          ...
                          'event_times_multivariate': event_times_multivariate[
                                                      np.sum(num_samples_[:i + 1]):np.sum(num_samples_[:i + 2])],
                          'event_types_multivariate': event_types_multivariate[
                                                      np.sum(num_samples_[:i + 1]):np.sum(num_samples_[:i + 2])],
                          'event_intervals_multivariate': event_intervals_multivariate[
                                                          np.sum(num_samples_[:i + 1]):np.sum(num_samples_[:i + 2])],
